...
val MIGRATION_1_2 = object : Migration(1, 2) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL("CREATE TABLE IF NOT EXISTS `personal-records` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
            "`stroke` TEXT NOT NULL, `distance` INTEGER NOT NULL)")
        ...
        database.execSQL("CREATE TABLE IF NOT EXISTS `record-time` " +
            "(`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `recordID` INTEGER NOT NULL, " +
            "`date` INTEGER NOT NULL, `unitOfMeasure` TEXT NOT NULL, " +
            "`hours` INTEGER NOT NULL, `minutes` INTEGER NOT NULL, " +
            "`seconds` INTEGER NOT NULL, `milliseconds` INTEGER NOT NULL, " +
            "FOREIGN KEY(`recordID`) REFERENCES `personal-records`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE )")
